export interface DescribeScalingGroupDetailResponse {
    /**
     * 请求ID。
    ...
    ScalingGroup: {
        ...
        /**
         * 当`MultiAZPolicy`取值为`COST_OPTIMIZED`时，如果因价格、库存等原因无法创建足够的抢占式实例，是否允许自动尝试创建按量实例满足ECS实例数量要求。可能值：
         * - true：允许。
         * - false：不允许。
         * @example `true`
         */
        ...
        /**
         * 伸缩组内实例的期望数量，伸缩组会自动将ECS实例数量维持在期望实例数。
         * @example `5`
         */
        ...
        AlbServerGroups: {
            ...
            /**
             * 弹性伸缩将实例添加到ALB服务器组后，实例作为后端服务器的权重。
             * @example `100`
             */
            Weight: number;
            /**
             * 弹性伸缩将实例添加到ALB服务器组后，实例使用的端口号。
             * @example `80`
             */
            Port: number;
        }[];
    ...
    /**
     * ECI伸缩组输出详情。目前仅支持Kubernetes Deployment YAML格式输出。
