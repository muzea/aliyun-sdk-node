export interface CreateEdgeDriverVersionRequest {
    /**
    ...
    "DriverId": string;
    /**
     * 驱动版本号。必须是该驱动唯一的版本号，即一个驱动不可以设置两个相同的版本号。允许中文、英文字母、数字、下划线（_）、短划线（-）和英文句号（.），长度不超过64个字符。
     * @example `1.2.0`
     */
    ...
    "DriverConfig"?: string;
    /**
     * 容器配置。JSON格式字符串。更多信息，请参见本文下方**ContainerConfig**表格。
